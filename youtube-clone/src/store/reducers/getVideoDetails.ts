...
export const getVideoDetails = createAsyncThunk(
  ...
  async (id: string) => {
    const {
      ...
      `${YOUTUBE_API_URL}/videos?key=AIzaSyBvpfdBygQ4-nLFBjwGMdEo2Q7UgRzG4Ng&part=snippet,statistics&type=video&id=${id}`
    );
...
const parseData = async (item: {
  ...
  const {
    ...
    `${YOUTUBE_API_URL}/channels?part=snippet,statistics&id=${item.snippet.channelId}&key=AIzaSyBvpfdBygQ4-nLFBjwGMdEo2Q7UgRzG4Ng`
  );
// ${API_KEY}
  return {
    videoId: item.id,
